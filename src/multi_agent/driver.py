...
def dispatch_visible(
    ...
    lines = [
        ...
        "text = ''",
        "try:",
        "    with open(tmplog) as _f: text = _f.read()",
        ...
        "            with open(outbox,'w') as _f: json.dump(d, _f, ensure_ascii=False, indent=2); sys.exit(0)",
        ...
        "summary = ', '.join(changed[:10]) if changed else f'codex exited {exit_code}, no files modified'",
        "with open(outbox,'w') as _f:",
        "    json.dump({'status': status, 'summary': summary, 'changed_files': changed}, _f, ensure_ascii=False, indent=2)",
        "PYEOF",
        '        rm -f "$TMPLOG"',
        "    fi",
        "",
